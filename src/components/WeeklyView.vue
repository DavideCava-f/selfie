...
<template>
  <div class="container-fluid border border-danger w-100 p-0 m-0">
    <div class="d-flex justify-content-between bg-light text-center rounded-3 p-0 w-100">
      ...
      <div v-for="day in store.week" class="row w-100 border fillable align-self-center align-items-center p-0"
        ...
        <div class="h-100 d-flex flex-row justify-content-between border-start gap-1 flex-wrap" style="width: 88%;">
          ...
          <button v-for="pomodoro in store.pomodoros.filter((p) => Temporal.PlainDate.compare(Temporal.PlainDate.from(p.beginDate.split('T')[0]),
            ...
            @click="store.toggle = !store.toggle; store.activePomodoro = pomodoro;" data-bs-target="#PomodoroEventModal"
